...
switches to **element mode**: it captures just the element and produces a focused
**`element.md`** instead of a full `design.md`.

The element is classified into one of three kinds, which determines what you get:

| Kind | Example | Output |
|---|---|---|
| `code` | Navbar, card, button, hero section | Scoped token spec + ready-to-paste reconstruction prompt for v0 / Claude Code / Lovable |
| `asset` | 3D illustration, mascot, photographic art | **Token-grounded prompt for generative image models** (gpt-image, Midjourney, Stable Diffusion) |
...
generic pretty image. On URLs, element capture is DOM-precise:
`capture_site.py --selector "header.navbar"` screenshots only the element and saves its
`outerHTML`.

See a real run in
[`examples/poolsuite-player-element/`](./examples/poolsuite-player-element) — element
mode pointed at the retro Mac OS player window of
[poolsuite.net](https://poolsuite.net), producing a hybrid `element.md` with the
...
And the **asset path** — element mode pointed at visual art. The clay-style 3D hero
scene of discord.com became a token-grounded image prompt
([`examples/discord-hero-asset/`](./examples/discord-hero-asset)); an image model
turned that prompt into the right image **on the first pass** — indigo world locked,
every accent on its extracted hex, original characters per the IP guardrail:

| Captured element (discord.com) | Generated from the `element.md` prompt |
|---|---|
...
And the `isolated` delivery format — the same extraction asked for as a web-ready
cutout: single subject, **real alpha**, no scenery, no ground shadow. Drop it
straight into your site:

<p align="center">
  <img src="./examples/discord-hero-asset/element-3D-isolated.png" width="280" alt="The robot companion as an isolated transparent PNG generated from the element.md prompt">
</p>
...
| `capture_site.py` | Multi-viewport Playwright screenshots with cookie banner auto-dismiss, scroll-capture for lazy content, and single-element capture (`--selector`) | `playwright` |
