...
- **Chinese (CJK) text renders in the wrong font / shows non-Simplified glyphs:** the cloud render environment may not ship the CJK fonts in the fallback chain. The bundle's `sans`/`mono` stacks already prioritize `Noto Sans SC` (cloud-friendly) ahead of `PingFang SC`. To guarantee it, load Noto Sans SC explicitly in the generated HTML `<head>`:
